...
export default function FundingPageClient({
  ...
  const handleProjectSelect = (projId: string | null) => {
    ...
    if (projId) {
      router.push(`/funding?projectId=${projId}`);
    } else {
      router.push('/funding');
    }
  };
    ...
    <div className="flex w-screen h-screen overflow-hidden bg-canvas font-dmsans">
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden">
        ...
        <div className="p-4 border-b border-hairline bg-canvas flex flex-col sm:flex-row sm:items-center justify-between gap-4 shrink-0 text-left">
          <div>
            <h1 className="font-lora text-2xl font-normal text-ink tracking-tight">Grants & Funding Finder</h1>
            <p className="text-xs text-muted mt-1">
              Identify Indian government grant allocations, MSME schemes, incubators, accelerators, and compile draft proposals automatically.
            </p>
          </div>
        ...
        <div className="flex-1 overflow-hidden relative">
          {activeProjectId ? (
            <FundingDashboard projectId={activeProjectId} userId={userId} />
          ) : (
            <div className="absolute inset-0 flex flex-col items-center justify-center p-6 text-center max-w-sm mx-auto space-y-4">
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-soft leading-relaxed">
                  Select an active startup project in the dropdown above to scan matching grant opportunities and review draft application proposals.
                </p>
              </div>
            </div>
          )}
