...
import DDDashboard from '../../components/due-diligence/DDDashboard';
import { Project } from '../../types';
import { AlertCircle } from 'lucide-react';

interface DDPageClientProps {
  initialProjectId: string | null;
  allProjects: Project[];
}

export default function DDPageClient({
  initialProjectId,
  allProjects,
}: DDPageClientProps) {
  ...
  const handleProjectSelect = (projId: string | null) => {
    ...
    if (projId) {
      router.push(`/due-diligence?projectId=${projId}`);
    } else {
    ...
    <div className="flex w-screen h-screen overflow-hidden bg-canvas font-dmsans">
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden">
        ...
        <div className="p-4 border-b border-hairline bg-canvas flex flex-col sm:flex-row sm:items-center justify-between gap-4 shrink-0 text-left">
          <div>
            <h1 className="font-lora text-2xl font-normal text-ink tracking-tight">AI Due Diligence Engine</h1>
            <p className="text-xs text-muted mt-1">
              Analyze startups through a parallel multi-agent research pipeline, compiling risk signals, TAM, and PDF briefs.
            </p>
          </div>
        ...
        <div className="flex-1 overflow-hidden relative">
          {activeProjectId ? (
            <DDDashboard projectId={activeProjectId} userId={userId} />
          ) : (
            <div className="absolute inset-0 flex flex-col items-center justify-center p-6 text-center max-w-sm mx-auto space-y-4">
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-soft leading-relaxed">
                  Select an active startup project in the dropdown above to create, view, or export due diligence investment reports.
                </p>
              </div>
            </div>
          )}
